...
This design document specifies the NICo Core secret-storage subsystem: a configurable credential chain that introduces a Postgres-backed, envelope-encrypted secret store alongside the existing Vault/OpenBao store, together with the key-management layer that protects stored secrets. The default configuration reproduces today's behavior exactly, so the subsystem is adopted by opt-in and is reversible at each step.
...
| DEK | Data Encryption Key — a fresh 256-bit key generated per write that encrypts one secret value |
| KEK | Key Encryption Key — the key that wraps (encrypts) a DEK; never applied to the value directly |
...
| `kek_id` | Opaque string naming a KEK; recorded on every stored row so a read can find the KEK that wrapped its DEK |
| Backend | A credential store that answers reads and (for the writer) accepts writes: Postgres or Vault |
...
| Transit | The Vault/OpenBao server-side cryptography engine that wraps and unwraps DEKs without exposing key material ([OpenBao Transit docs](https://openbao.org/docs/secrets/transit/)) |
...
* Callers address a credential by a stable path string (`CredentialKey::to_key_str()`) that is identical across the Vault and Postgres backends.
...
* The credential chain is built once at boot from the `[secrets]` configuration, or — when that section is absent — from the legacy `CARBIDE_CREDENTIAL_STORE` path. Local-override readers (env, file), when enabled, are consulted ahead of the configured backends.
...
* The KEK layer presents a single `KmsBackend` (a `MultiKmsProvider`): it wraps new DEKs with the active provider and unwraps any row through whichever provider owns that row's `kek_id`.
...
3. **KEK rotation** — an operator changes routing (the KEK that new writes use) and runs `nico-admin-cli secrets re-wrap`; the server walks the journal and re-wraps each stale row's DEK under the active KEK, leaving the value ciphertext untouched. An old KEK is retired once no rows reference it.
...
| Database (Postgres) | Hosts the append-only `secrets` table, and provides the advisory locks that make create, import, and re-wrap safe across replicas |
...
There are five operational areas associated with this subsystem:
...
Each of these flows is discussed below, followed by the data model, configuration, the management API, and the design decisions behind them.

## 3.1 Write path (envelope encryption)

The write path turns a plaintext credential into a self-describing encrypted row. The writer asks routing for the path's active `kek_id`, generates a fresh 256-bit DEK and wraps it with that KEK, encrypts the value with the DEK (AES-256-GCM, AAD = path), and appends the row to the journal. `create_credentials` encrypts before the transaction, then takes a per-path advisory lock, checks for an existing path, and inserts — refusing a duplicate; `set_credentials` appends unconditionally.
...
![Path-to-KEK routing for writes and the re-wrap walk that rotates existing rows](kek-routing-rewrap-flow.png)
...
| `kek_id` | `TEXT` | no | The KEK that wrapped this row's DEK; indexed, drives re-wrap |
...
[secrets.kms]
active = "local"                   # provider that wraps new DEKs
...
A KEK migration adds a second provider that still owns the old `kek_id` (so historical rows keep decrypting) while the active provider owns the new one; `secrets re-wrap` then sweeps rows onto the new KEK, after which the old provider can be removed.
...
* **`MultiKmsProvider`: active writer + owner-routed reads.** One active provider wraps new DEKs; reads route to the first provider that owns a row's `kek_id`. Startup validates that every routed `kek_id` is owned by the active provider and that no `kek_id` is owned by two providers, so routing is unambiguous.
...
* Deletion is backend-local. A tombstone or `delete_credentials` lands only in the writer backend; in a multi-backend chain, a lower-priority backend that still holds the path answers reads for it again. During a migration window, delete from both stores — or complete the cutover — to make a deletion stick.
