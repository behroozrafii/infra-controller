...
impl NmxcTlsConfig {
    ...
    /// the next connect surfaces that as an error.
...
impl NmxcClientPool {
    ...
    /// a benign duplicate handshake. The production caller is a serial monitor
    /// loop, so per-URI in-flight serialization would be machinery without a
    /// workload.
