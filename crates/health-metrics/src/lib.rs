    ...
    F: Fn(&T) -> &HealthIterationMetrics<D> + Send + Sync + 'static,
{
    ...
    // {prefix}_health_overrides_count
    {
        ...
        meter
            ...
            .with_description("Number of health overrides that are configured in the site")
            .with_callback(move |observer| {
                metrics.if_available(|metrics, attrs| {
                    let health = project(metrics);
    ...
    // {prefix}_unhealthy_by_classification_count
    {
        ...
        meter
            ...
            .with_description(
                "Number of objects which are marked with a certain classification due to being unhealthy",
            )
            .with_callback(move |observer| {
                metrics.if_available(|metrics, attrs| {
                    let health = project(metrics);
