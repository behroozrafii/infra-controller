...
func (m *ManagerImpl) Cancel(
	...
	if cancelApplied {
		...
		m.cancelTasks(ctx, result.ID, canceller)
	}
...
func (m *ManagerImpl) continueOperationRun(
	...
	return func(
		...
		summary, err := m.ensureSafetyGatesAllowRun(
			txCtx,
			run,
			expectedPhaseIndex,
		)
...
func (m *ManagerImpl) ensureSafetyGatesAllowRun(
	ctx context.Context,
	run *operationrun.OperationRun,
	expectedPhaseIndex int32,
	...
	if err != nil {
		return operationrun.TargetPhaseSummary{}, err
	}
	...
	// Manual controls should not report success when the locked target
	// state already trips a safety gate. The dispatcher still rechecks
	// gates before it starts any additional tasks.
	if err := safetyGatesAllowRun(run, summary); err != nil {
		if updateErr := m.store.UpdateRunState(ctx, run); updateErr != nil {
			return summary, fmt.Errorf("%w: update safety pause: %v", err, updateErr)
		}
		return summary, err
	}
...
func safetyGatesAllowRun(
	run *operationrun.OperationRun,
	summary operationrun.TargetPhaseSummary,
) error {
	options, err := run.DecodedOptions()
	if err != nil {
		return fmt.Errorf("decode operation run options: %w", err)
	}

	evaluation := summary.EvaluateSafetyGates(options.SafetyPolicy.Gates)
	if evaluation.Tripped {
		...
		return fmt.Errorf(
			"%w: %s",
			ErrOperationRunSafetyGateTripped,
			evaluation.Message,
		)
	}
